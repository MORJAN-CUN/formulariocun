...
foreach($data_empleados as $data){
	...
	$sheet->setCellValue('C'.$fila, $consecutivo);
	...
	$sheet->setCellValue('AB'.$fila, strtolower($data['TR_ALTO_RIESGO_PENSION']));
	...
	$sheet->setCellValue('AG'.$fila, strtolower($data['TR_SALARIO_INTEGRAL']));
	...
	$DyD_devengados_JSON = json_encode($DyD_devengados, JSON_NUMERIC_CHECK | JSON_UNESCAPED_SLASHES);
	$DyD_deducciones_JSON = json_encode($DyD_deducciones, JSON_NUMERIC_CHECK | JSON_UNESCAPED_SLASHES);
