...
$url = 'http://190.184.202.251:8090/formularioback/Admin/recibos_full/ConsOrdenesCedula.php';
//$url = 'http://localhost/CUN/formularioback/Admin/recibos_full/ConsOrdenesCedula.php';
...
<?php 
	...
	foreach ($data as $key){
		...
		$aleatorio = substr(str_shuffle(str_repeat("0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ", 10)), 0, 10); 
		...
		<tr>
	       ...
	       <td data-label="Name" style="font-size:11px;" >
			...
			<td>
				...
				<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#gestionModal<?php echo $key['CLIENTE_SOLICITADO'].$aleatorio; ?>">
					&curren;
				</button>

				<!-- Modal -->
				<div class="modal fade modG" id="gestionModal<?php echo $key['CLIENTE_SOLICITADO'].$aleatorio; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
						...
						<div class="modal-body">
							<div class="row">
								...
								<div class="col-md-6">
									...
									<?php if (in_array("AJO", $accesos_arr)) { ?>
    ...
    <script>
        var jsonData = <?php echo "'" . str_replace("'", "", json_encode($key)) . "'"; ?>;
    </script>

    <input type="button" value="✓" class="btn btn-outline-info" onclick="MarcaPago(JSON.parse(jsonData));">
<?php } ?>
								...
								<div class="col-md-6">
									...
									<?php if (in_array("QCO", $accesos_arr)) { ?>
 <script>
        var jsonData = <?php echo "'" . str_replace("'", "", json_encode($key)) . "'"; ?>;
    </script>
										<input type="button" value="✓" class="btn btn-outline-info" onclick='QuitarMarcaPago(JSON.parse(jsonData));'>
									<?php } ?>
								</div>
								<?php 
								if($key['CUNVIVE'] == 'ACTIVADO'){
								?>
								<?php
								} else {
								?>
						...
						<div class="modal-footer">
							<button type="button" class="btn btn-danger" onclick="$('#gestionModal<?php echo $key['CLIENTE_SOLICITADO'].$aleatorio; ?>').modal('hide')">Cerrar</button>
						</div>
						</div>
					</div>
				</div>
